...
int main(void) {
    ...
    if (raft_cluster_init(cluster) != 0) {
        return 1;
    }

    rx_trace_init(&trace, 1);
    if (raft_cluster_attach_trace(cluster, &trace) != 0) {
        return 1;
    }
    ...
    raft_cluster_add_node(cluster, &n1, "var/c-trace-example");
    raft_cluster_add_node(cluster, &n2, "var/c-trace-example");
    raft_cluster_add_node(cluster, &n3, "var/c-trace-example");
    ...
    raft_cluster_add_node(cluster, &n4, "var/c-trace-example");
